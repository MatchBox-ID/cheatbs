...
os.system("clear")
autoketik(f"{biru}[{kuning}Warning{biru}] {W}Jangan Lupa Subscribe Channel Brawl Stars")
time.sleep(3)
os.system("xdg-open https://youtube.com/c/Brawlstars")
...
{merah}╰━┻╯╰┻━┻╯╰╯╰╯ {merah}
{abu}-----------------------------------------
{putih}[{biru}•{putih}] {biru}Author {putih}   : Brawl Stars X Matchbox
{putih}[{biru}•{putih}] {abu}GitHub {putih}   : Matchbox - ID
{putih}[{biru}•{putih}] {merah}You{putih}Tube {putih}  : Brawl Stars
{putih}[{biru}•{putih}] {ungu}Instagram {putih}: @Brawlstars
{W}[{Y}•{W}] Ip Kamu {putih}  :{Y} {ip}
{W}[{Y}•{W}] Waktu/Jam {putih}:{Y} {localtime}
{W}[{Y}•{W}] Total Run {putih}:{Y} {getvisit['value']}
""")

nomor = input(f"{W}[{R}• {kuning}•{hijau}•{W}] {biru}Jumlah Cheat Gems{W}: ")
jum = int(input(f"{W}[{R}• {kuning}•{hijau}•{W}] {biru}Jumlah Cheat Koin{W}: "))
cheat = int(input(f"{W}[{R}• {kuning}•{hijau}•{W}] {biru}Ussername{W}: "))
for i in range(jum):
    ...
    print (f"{W}[{G}✓{W}] Success Cheat Your Account")
    print (f"{W}[{G}✓{W}] Gems & Koin Telah Masuk Ke Akun Anda")
